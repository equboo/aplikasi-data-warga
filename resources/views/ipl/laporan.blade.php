        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-lg font-bold mb-4">Laporan Arus Kas Periode: {{ \Carbon\Carbon::create()->month($bulan)->translatedFormat('F') }} {{ $tahun }}</h3>
                    <form method="GET" action="{{ route('ipl.laporan') }}" class="flex flex-col sm:flex-row gap-4 items-center p-4 bg-gray-50 rounded-lg mb-6">
                        <div class="flex-grow">
                            ...
                            <select name="bulan" id="bulan" class="mt-1 w-full border-gray-300 rounded-md shadow-sm">
                                @foreach ($months as $num => $name)
                                    <option value="{{ $num + 1 }}" @selected($bulan == $num + 1)>{{ $name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="flex-grow">
                            <x-input-label for="tahun" value="Pilih Tahun" />
                            <select name="tahun" id="tahun" class="mt-1 w-full border-gray-300 rounded-md shadow-sm">
                                ...
                                @if(!in_array(now()->year, $years->toArray()))
                                     <option value="{{ now()->year }}" @selected($tahun == now()->year)>{{ now()->year }}</option>
                                @endif
                            </select>
                        </div>
                        <div class="self-end flex gap-2">
                            <x-primary-button class="mt-1">Tampilkan</x-primary-button>
                            <a href="{{ route('laporan.cetak', request()->query()) }}" target="_blank" class="mt-1 inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700">
                                Cetak Laporan
                            </a>
                        </div>
                    </form>
